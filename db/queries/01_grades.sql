...
ORDER BY
    ...
    CASE WHEN @sort_by = 'grade' AND @sort_dir = 'asc' THEN (CAST(g.earned AS FLOAT) / NULLIF(g.total, 0)) END ASC NULLS FIRST,
    CASE WHEN @sort_by = 'grade' AND @sort_dir = 'desc' THEN (CAST(g.earned AS FLOAT) / NULLIF(g.total, 0)) END DESC NULLS FIRST,
    -- Sort by date
